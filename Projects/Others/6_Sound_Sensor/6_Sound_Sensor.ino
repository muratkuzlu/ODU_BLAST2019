...
int  sensorDigitalPin = D6;    // Select the Arduino input pin to accept the Sound Sensor's digital output
int  analogValue = 0;         // Define variable to store the analog value coming from the Sound Sensor
int  digitalValue;            // Define variable to store the digital value coming from the Sound Sensor
int  Led13 = D1;              // Define LED port; this is the LED built in to the Arduino (labled L)
...
void loop(){
  ...
  Serial.println(analogValue); // Send the analog value to the serial transmit interface
    Serial.println(digitalValue); // Send the analog value to the serial transmit interface
